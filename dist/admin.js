...
class SquigAdmin extends _Squig__WEBPACK_IMPORTED_MODULE_0__["Squig"] {
  constructor() {
    ...
    this.fillTable = () => {
      ...
      var tableTimeBody = this.tableTime.getElementsByTagName("tbody")[0];
      tableTimeBody.innerHTML = "";
      var tableUserBody = this.tableUser.getElementsByTagName("tbody")[0];
      ...
      var ids = Object.keys(this.lines).concat(Object.keys(this.texts)).sort((a, b) => +b - +a);

      var _loop = function _loop(i) {
        var id = ids[i];
        var line = _this.lines[+id] || _this.texts[+id];
        ...
        btnDel.addEventListener("click", () => _this.socket.emit("delete-line", {
          id
        }));
      ...
      for (var _i = 0; _i < usersIDs.length; _i++) {
        _loop2(_i);
      }
    };
  ...
  drawSelectedText(text) {
    var ctx = this.ctx;
    ctx.save();
    ctx.fillStyle = "rgba(255, 255, 100, 0.5)";
    ctx.font = "50px Calibri, sans-serif";
    ctx.textBaseline = "top";
    var tm = ctx.measureText(text.text);
  ...
  initSocket() {
    ...
    socket.on("connect", () => {
      ...
      socket.on("imgs", list => {
        var imgsDiv = document.getElementById("options-img");
        imgsDiv.innerHTML = "";

        var handleClick = e => {
          var img = e.currentTarget;
          var path = img.src;
          var dim = [720 * img.width / img.height, 720];
          this.adjustSize(...dim);
          socket.emit("dim", dim);
          socket.emit("new-img", {
            path
  ...
  redraw() {
    ...
    for (var _id in lines) {
      var line = lines[_id];
      if (this.selected.indexOf(_id) !== -1) this.drawSelectedLine(line);
      this.drawLine(line);
    }

    for (var _id2 in texts) {
      var text = texts[_id2];
      if (this.selected.indexOf(_id2) !== -1) this.drawSelectedText(text);
      this.drawText(text);
    }
