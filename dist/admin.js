...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.squig = squig;
  var squigAdmin = {};
  squigAdmin.tableTime = document.getElementById("table-time");
  squigAdmin.tableUser = document.getElementById("table-user");
  squigAdmin.selected = [];
  ...
  var draw = () => {
    ...
    for (var _id in lines) {
      var line = lines[_id];
      if (squigAdmin.selected.indexOf(_id) !== -1) drawSelectedLine(ctx, line);
      drawLine(ctx, line);
    }
  ...
  var fillTable = () => {
    ...
    var tableTimeBody = squigAdmin.tableTime.getElementsByTagName("tbody")[0];
    tableTimeBody.innerHTML = "";
    var tableUserBody = squigAdmin.tableUser.getElementsByTagName("tbody")[0];
    ...
    var handleDeselect = id => {
      var i = squigAdmin.selected.indexOf(id);
    ...
    var ids = Object.keys(squig.lines).sort((a, b) => +b - +a);

    var _loop = function _loop(i) {
      var id = ids[i];
      var line = squig.lines[+id];
      var time = new Date(+id);
      var dMin = (curTime.getTime() - time.getTime()) / 60000;
      var color = line.color;
      var user = line.user.substr(0, 6);
      ...
      btnDel.addEventListener("click", () => squig.socket.emit("delete-line", {
        id
      }));
    ...
    var usersIDs = Object.keys(users).sort((a, b) => +users[a].last - +users[b].last);
  ...
  setInterval(fillTable, 60000); // Socket init
